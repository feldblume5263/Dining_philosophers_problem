...
/*   Updated: 2021/05/15 04:37:07 by junhpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sema_init(t_info *info)
{
	...
	info->forks = sem_open("/forks", O_CREAT | O_EXCL, 0755, g_philo_num);
		// idx++;
	// }
	info->print_sem = sem_open("/print_sema", O_CREAT | O_EXCL, 0755, 1);
	info->eating_philo = sem_open("/eating_pilo", O_CREAT | O_EXCL, 0755, (g_philo_num / 2));
	// sem_init(&(info->print_sem), 0, 1); // 마찬가지
}
